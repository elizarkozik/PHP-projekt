  ...
  <div class="collapse navbar-collapse" id="navbarSupportedContent">
    <ul class="navbar-nav mr-auto">
     <!-- <li class="nav-item active">
        <a class="nav-link" href="my_message.php">? <span class="sr-only">(current)</span></a>
      </li>-->
...
if(isset($_POST['create_entry'])){
  ...
  if(empty($title) || empty($date) || empty($categories) || empty($entry_text)){
    header("Location: my_message.php?entries=empty");
    die();
  }
...
if (move_uploaded_file($_FILES['image']['tmp_name'], $image_file)){
  ...
  if($stm->execute()) {
    header("Location: my_message.php?entries=success");
  } else {
    header("Location: my_message.php?entries=error");
  }
...
<div class="login-page">
  <div class="form">
  <h3>Write message</h3>
    <form class="login-form" action="my_message.php" method="POST" enctype="multipart/form-data">
      <input type="text" placeholder="title" name="title"/>
      <input type="date" name="date"/>
      <select name="categories" id="select">
      <option value="1">Clothes</option>
      <option value="2">Accesoaries</option>
      <option value="3">Interior</option>
      </select>
      <input type="file" name="image">
      <textarea name="entry_text" id="textarea" cols="30" rows="10" placeholder="Write your message here..."></textarea>
      <input id="button" type="submit" value="Write message" name="create_entry">
    </form>
    <?php 
if(!isset($_GET['entries'])) {
  ...
  if($entries == "empty"){
    echo "<p class='error_form'> Please fill in all fields </p>";
    die();
  }
    elseif($entries == "success"){
    /*echo "<p class='success_form'> You have create your account</p>";*/
    header("location:index.php");
    die();
  } elseif($entries == "error"){
    echo "<p class='error_form'> Something wrong</p>";
    die();
  }
